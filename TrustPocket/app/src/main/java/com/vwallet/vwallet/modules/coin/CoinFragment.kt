package com.vwallet.vwallet.modules.coin
...
import com.vwallet.vwallet.modules.coin.adapters.*
import com.vwallet.vwallet.modules.markdown.MarkdownFragment
import com.vwallet.vwallet.modules.market.overview.PoweredByAdapter
import com.vwallet.vwallet.modules.metricchart.MetricChartFragment
import com.vwallet.vwallet.modules.metricchart.MetricChartType
import com.vwallet.vwallet.modules.settings.notifications.bottommenu.BottomNotificationMenu
import com.vwallet.vwallet.modules.settings.notifications.bottommenu.NotificationMenuMode
...
class CoinFragment : BaseFragment(), CoinChartAdapter.Listener, CoinDataAdapter.Listener, CoinLinksAdapter.Listener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        var linksLiveData: MutableLiveData<List<CoinLink>>
        var items = listOf<CoinLink>()

        viewModel.linksLiveData.observe(viewLifecycleOwner) { list ->
            if (list.isEmpty()){
                return@observe
            }
        ...
        toolbar.setOnMenuItemClickListener { item ->
            when (item.itemId) {
                ...
                R.id.menuWeb -> {
                    for(item in items){
                        if( item.linkType==LinkType.WEBSITE){
                            context?.let { ctx ->
                                LinkHelper.openLinkInAppBrowser(ctx, item.url.trim())
                            }
                            break;
                        }
                    }
    ...
    private fun observeData() {
        ...
        viewModel.isFavorite.observe(viewLifecycleOwner, Observer { isFavorite ->
            ...
            /*if(COIN_CAT_KEY==MarketCategory.Dexes.toString()){
                toolbar.menu.findItem(R.id.menuWeb).isVisible = true
            }else{
                toolbar.menu.findItem(R.id.menuWeb).isVisible = false
